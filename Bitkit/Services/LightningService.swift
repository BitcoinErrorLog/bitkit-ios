...
public class LightningService {
    ...
    func sendWithRetry(bolt11: String, sats: UInt64? = nil, params: RouteParametersConfig? = nil) async throws -> PaymentHash {
        let maxRetries = 3
        let maxTotalTime: TimeInterval = 12
        let startTime = Date()
        ...
        for attempt in 1...maxRetries {
            do {
                return try await send(bolt11: bolt11, sats: sats, params: params)
            } catch {
                lastError = error
                
                if !isRetryableError(error) {
                    Logger.debug("Payment error is not retryable, failing immediately", context: "LightningService")
                    throw error
                }
                
                if Date().timeIntervalSince(startTime) > maxTotalTime {
                    Logger.debug("Payment retry time budget exhausted", context: "LightningService")
                    throw error
                }
                
                let baseDelay: TimeInterval = attempt == 1 ? 1.0 : (attempt == 2 ? 3.0 : 7.0)
                let jitter = Double.random(in: 0...0.5)
                let delayNanoseconds = UInt64((baseDelay + jitter) * 1_000_000_000)
                
                Logger.info("Payment attempt \(attempt) failed, retrying in \(baseDelay + jitter)s...", context: "LightningService")
                try await Task.sleep(nanoseconds: delayNanoseconds)
            }
        }
        
        throw lastError ?? AppError(message: "Payment failed after \(maxRetries) retries", debugMessage: nil)
    }
